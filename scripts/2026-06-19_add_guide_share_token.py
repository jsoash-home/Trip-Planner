#!/usr/bin/env python3
"""
scripts/2026-06-19_add_guide_share_token.py

One-off migration: adds the `guide_share_token` column and its unique
index to the `trip` table in the production SQLite database.

Usage:
    cd "/Users/jeff_s/Projects/Vacation Planner"

    # Review what will happen (default: interactive prompt):
    .venv/bin/python scripts/2026-06-19_add_guide_share_token.py

    # Non-interactive (CI / scripted):
    .venv/bin/python scripts/2026-06-19_add_guide_share_token.py --yes
...
Rollback (SQLite 3.35+ required for DROP COLUMN):
    ...
    Then:
        DROP INDEX IF EXISTS ix_trip_guide_share_token;
...
COLUMN_NAME = "guide_share_token"
TABLE_NAME = "trip"
INDEX_NAME = "ix_trip_guide_share_token"

ALTER_SQL = (
    f"ALTER TABLE {TABLE_NAME} ADD COLUMN "
    f"{COLUMN_NAME} VARCHAR(36)"
)
INDEX_SQL = (
...
def _column_exists(conn: sqlite3.Connection) -> bool:
    """Return True if guide_share_token already exists in the trip table."""
    rows = conn.execute(f"PRAGMA table_info({TABLE_NAME})").fetchall()
    names = [row[1] for row in rows]
    return COLUMN_NAME in names


def _column_count(conn: sqlite3.Connection) -> int:
    """Return the number of columns currently in the trip table."""
    rows = conn.execute(f"PRAGMA table_info({TABLE_NAME})").fetchall()
    return len(rows)
...
def _confirm_interactively(db_path: Path) -> bool:
    """Print a warning and return True only if the user types 'y'."""
    print()
    print(f"About to alter {db_path} — proceed? [y/N] ", end="", flush=True)
    try:
...
def run_migration(db_path: Path, yes: bool) -> int:
    ...
    try:
        if _column_exists(conn):
            logger.info(
                "Already migrated: column '%s' exists in table '%s'. Nothing to do.",
                COLUMN_NAME,
                TABLE_NAME,
            )
            return 0

        before_count = _column_count(conn)
    finally:
        conn.close()
    ...
    # ── 3. Confirmation ─────────────────────────────────────────────────
    if not yes:
        if not _confirm_interactively(db_path):
    ...
    logger.info(
        "Done. trip table: %d column(s) → %d column(s). New column: %s. "
        "New index: %s.",
        before_count,
        after_count,
        COLUMN_NAME,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Add guide_share_token column to trip table in vacation.db."
    )
    parser.add_argument(
        "--yes",
        action="store_true",
